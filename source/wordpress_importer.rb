...
module Jekyll
  module WordPress
    ...
    # post in posts that has post_status = 'publish'.
    # This restriction is made because 'draft' posts are not guaranteed to
    # have valid dates.
    QUERY = "select post_title, post_name, post_date, post_content, post_excerpt, ID, guid from posts where post_status = 'publish' and post_type = 'post'"
    # Fetch all tags for a given POST ID
    TAGS_QUERY = "select tm.term_id,tm.name from term_relationships tr 
                  inner join term_taxonomy tt on tr.term_taxonomy_id = tt.term_taxonomy_id 
                  inner join terms tm on tm.term_id=tt.term_id  
    ...
    def self.process(dbname = '', user='', pass='', host = 'localhost', domain = 'www.robgolding.com')
      ...
      db[QUERY].each do |post|
        ...
        slug = post[:post_name]
        date = post[:post_date]
        content = self.wrap_text(DownmarkIt.to_markdown post[:post_content])
    ...
    def self.transformUrls(domain,content)
        ...
        return content.gsub(baseurl,"cdn.robgolding.com/uploads/")
    end
